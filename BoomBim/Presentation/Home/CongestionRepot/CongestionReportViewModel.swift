...
//    private let selectedLevel = BehaviorRelay<Int?>(value: nil)
//    
//    struct Input {
//        let postTap: Signal<Void>
//        let levelSelect: Signal<Int>   // UI에서 선택되는 레벨 스트림
//    }
//    struct Output {
//        let completed: Signal<ReportData>
//        let loading: Driver<Bool>
//        let error: Signal<String>
//    }
//    
...
//        let loadingRelay = BehaviorRelay<Bool>(value: false)
//        let errorRelay = PublishRelay<String>()
//        let completeRelay = PublishRelay<ReportData>()
//        
//        input.levelSelect
//            .emit(to: selectedLevel)  // latest level 저장
//            .disposed(by: disposeBag)
//        
//        let latestLevel = selectedLevel.asObservable()
//        let latestPlace = Observable.deferred { [weak self] in
//            .just(self?.currentSelectedPlace) // Place?
//        }
//
//        // (place, level) 상태를 묶어놓고, 탭 순간에 샘플링
//        let state = Observable.combineLatest(latestPlace, latestLevel) // (Place?, Int?)
//        
//        input.postTap
//            .throttle(.milliseconds(700))
//            .asObservable()
//            .withLatestFrom(state) // <- 핵심
//            .flatMapLatest { [weak self] placeOpt, levelOpt -> Observable<Event<ReportData>> in
//                guard let self else { return .empty() }
//                guard let place = placeOpt else {
//                    errorRelay.accept("장소를 선택해 주세요.")
//                    return .empty()
...
//
//                let body = PostPlaceRequest(
//                    memberPlaceId: Int(place.id),
//                    congestionLevelId: level,
//                    congestionMessage: "",                 // 메시지 있으면 채우기
//                    latitude: place.coord.latitude,
//                    longitude: place.coord.longitude
//                )
//
//                loadingRelay.accept(true)
//                return PlaceService.shared.postReport(body: body)
//                    .map { $0.data }                      // ReportData
//                    .asObservable()
//                    .materialize()                        // Observable<Event<ReportData>>
//            }
//            .observe(on: MainScheduler.instance)
//            .subscribe(onNext: { event in
//                switch event {
//                case .next(let data):                    // ✅ ReportData로 받기
//                    completeRelay.accept(data.memberPlaceId)
//                    loadingRelay.accept(false)
//                case .error(let err):
//                    errorRelay.accept(err.localizedDescription)
//                    loadingRelay.accept(false)
...
//        
////        input.postTap
////            .throttle(.milliseconds(700)) // 연타 방지
////            .asObservable()
////            .compactMap { [weak self] _ in (self?.currentSelectedPlace, self?.selectedLevel.value) } // 선택 장소 없으면 무시
////            .flatMapLatest { [weak self] place -> Observable<Event<ReportData>> in
////                guard let self else { return .empty() }
////                let body: PostPlaceRequest = .init(
////                    memberPlaceId: Int(place.id),
////                    congestionLevelId: selectedLevel,
////                    congestionMessage: <#T##String#>,
////                    latitude: <#T##Double#>,
////                    longitude: <#T##Double#>)
////                
////                loadingRelay.accept(true)
...
////                    .map { $0.data }
////                    .asObservable()
////                    .materialize() // success/error를 하나의 스트림으로
////            }
////            .observe(on: MainScheduler.instance)
////            .subscribe(onNext: { event in
////                switch event {
////                case .next(let memberPlaceId):
////                    completeRelay.accept(memberPlaceId)
////                    loadingRelay.accept(false)
////                case .error(let err):
////                    errorRelay.accept(err.localizedDescription)
////                    loadingRelay.accept(false)
...
//
//        return Output(
//            completed: completeRelay.asSignal(),
//            loading: loadingRelay.asDriver(),
//            error: errorRelay.asSignal()
//        )
//    }
